...
%
% Output:
%   - Saves U1_nodwell.mat containing X, U for later use.
...
for i=1:numSteps
    ...
    % ---------- Equation (5): Set τ(t)=0, for all times t ----------
    tr = 0; % τ(t)/dt
    tr_t = 0; % with respect to real seconds

    
    % ---------- Equation (7): compute state Xt=X(t+τ(t))=X(t), since  τ(t)=0, for all times t   ----------
    ...
    for j = max(1,i+tr-delay_steps_controller):i
     ...
     % Control input (4) with  τ(t)=0, for all times t
