...
%swsig_values = generateSwSignal(dwell_time,dwell_time_upper,T,dt);  %random switching signal 
...
for i=1:numSteps
    ...
    for j = max(1,i+tr-delay_steps_controller):i
        ...
        X(:, i+1) = X(:, i) + dt * ( A * X(:, i) + B * U(i) ); % Implementation of (1)
        Xt(:, i) = Pwr_A{swsig_values(switching)}^(tr_t/dt) * X(:, i) + integral_term_t*dt; % Implementation of (7)
        U(i+1+delay_steps) = K_bar *(Pwr_Abar^((Dcontroller-tr_t)/dt)* Xt(:, i) + integral_term*dt );  % Update control, implementation of (4)

end
save('U1_nodwell.mat', 'X', 'U');
...
% Plot the values of the switching signal with respect to time
figure('Position', [100, 100, 800, 600]);
