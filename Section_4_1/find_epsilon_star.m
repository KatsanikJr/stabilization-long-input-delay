...
% Average system Dynamics
A_bar = optimizeL2(A1, A2, A3);
B_bar = optimizeL2(B1, B2, B3);
K_bar = optimizeL2(K1, K2, K3);
...
eps = max([norm(A1-A_bar,2),norm(A2-A_bar,2),norm(A3-A_bar,2),norm(B1-B_bar,2),norm(B2-B_bar,2),norm(B3-B_bar,2),norm(K1-K_bar,2),norm(K2-K_bar,2),norm(K3-K_bar,2)]);
epsAb = max([norm(A1-A_bar),norm(A2-A_bar),norm(A3-A_bar),norm(B1-B_bar),norm(B2-B_bar),norm(B3-B_bar)]);
...
MA = max([norm(A1),norm(A2),norm(A3),norm(A_bar)]);
MB = max([norm(B1),norm(B2),norm(B3),norm(B_bar)]);
MK = max([norm(K1),norm(K2),norm(K3),norm(K_bar)]);
